...
用户要求以后周六8点自动运行，并立即生成本期三类周报。`weekly_reports.py` 从本期起以 Asia/Shanghai 周六08:00为截止与到期时间，下一期为2026-09-19 08:00。首期窗口为2026-09-06 23:00至2026-09-12 08:00（北京时间），与上一期已发布窗口连续，旧归档不变。后续固定七天，不受美西夏令时变化影响；用户随后确认月报也改为北京时间次月第一个周六08:00，详见 `monthly-reports.md` 最新节。
...
最终三类2026-09-12周报均已发布并核验，complete/notified均为true且无error；显式恢复入口退出0。投资只使用四篇正式日报，开发319条记录（两条旧登记路径的读取缺口已在正文披露），水利4条当前主仓提交。线上正文、封面和匿名保护经原发布引擎核验；本轮目视检查开发、水利封面/记录图及投资收益/缓冲图。月报时间变更后相关测试累计37项通过。
