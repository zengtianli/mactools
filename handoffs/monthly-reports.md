...
## 2026-09-12 改为次月第一个周六早上8点

用户明确确认月报也采用北京时间次月第一个周六08:00，三类一起自动运行。9月报下一次到期为2026-10-03 08:00。仍由已加载的 `com.tianli.reports` 每小时及加载时检查，保留归档、发布锁、完成状态和通知去重。

9月起按 Asia/Shanghai 完整自然月取材，历史8月及此前保持原美西窗口；这样月初恰逢周六时，08:00运行也已结束整月。Git时间戳在旧8月与新9月的时区切换处有15小时范围重叠，旧文不重写；投资交易日按原日报日期取整月，日期集合不变。`--period` 历史补期按同一月份分界选择时区。

真实入口 `weekly_reports.py --frequency monthly --check` 已回读next为2026-10-03T08:00:00+08:00。周/月相关37项测试通过，覆盖首次切换、到期前1秒、月初即周六、闰年和美西夏令时不影响新规则。下文为历史实现与首次发布记录。
