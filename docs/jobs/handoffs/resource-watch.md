...
- 9月15日最新用户纠正：自动弹窗改为整机 CPU **>95% 连续5秒**；刚好95%也重新计时。每秒只读取轻量CPU计数，完整进程/内存采集仍15秒，首次达到5秒立即补采并进入事件与弹窗判断。无效值、乱序或漏采重置；其他异常仅留证，主动查看仍可。独立配置 `prompt_cpu_threshold=95`、`prompt_sustained_seconds=5`，不受旧事件记录的60秒门槛限制，保留弹窗冷却与防重复。触发快照另存逐秒读数，区分较长区间的整机CPU平均值。
...
当前已安装二进制版本 `2026-09-15-cpu-5s`，经 launchd 重载运行。严格5秒门控、较长均值不阻挡快速资格、冷却与记录分离等策略127项通过，其他采样/存储/诊断/安全检查150项通过，共277项。触发条件用隔离采样序列验证，没有故意压满用户机器。此前完整采样实测约75–160ms（随负载变化）；本版每秒额外读取轻量内核CPU计数，完整采集频率保持15秒。
