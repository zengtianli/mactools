...
这里集中管理本人 Mac 自动更新、报告、提醒、同步与后台作业。独立自动化作业（如青龙）真身放在 `jobs/`；报告、投资、通知等已有业务实现保留在所属仓库，通过软链接接入，不复制脚本、配置和业务状态。
...
- `updates/`：软件更新脚本入口；`projects/`：原业务仓库链接；`qinglong/`：独立自动化项目真身。
- `archive/`：可回退的配置整理记录，不能当作活配置重新安装。
- `qinglong/`：青龙项目真身，含容器活数据与 CK 登录脚本；不是 `Apps` 下的独立应用。其项目指引和 catalog 在目录内。
